...
const Navbar = () => {
  ...
  return (
    <>
      <nav className='sticky top-0 z-40 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border-b border-white/30 dark:border-gray-700/50 shadow-sm transition-colors duration-300'>
        <div className='max-w-7xl mx-auto px-4 flex items-center justify-between h-16'>
          ...
          <ul className='hidden lg:flex items-center gap-6'>
            ...
            <li>
              <button onClick={() => token ? navigate('/rewards') : navigate('/login')}
                className='relative text-sm font-medium text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white transition-colors pb-1'>
                REWARDS
          ...
          <div className='hidden lg:flex items-center gap-2'>
            ...
            <button onClick={() => navigate('/admin')}
              className='px-4 py-1 bg-black dark:bg-white text-white dark:text-black rounded-md hover:bg-gray-800 dark:hover:bg-gray-100 transition-all text-sm'>
              Admin Panel
            </button>
          </div>
          ...
          <div className='flex items-center gap-1'>
            ...
            >
              {isDark ? (
                ...
                <svg className='w-5 h-5 text-yellow-400' fill='currentColor' viewBox='0 0 24 24'>
                  <path d='M12 2a1 1 0 0 1 1 1v1a1 1 0 1 1-2 0V3a1 1 0 0 1 1-1zm0 16a1 1 0 0 1 1 1v1a1 1 0 1 1-2 0v-1a1 1 0 0 1 1-1zm8-8a1 1 0 0 1 0 2h-1a1 1 0 1 1 0-2h1zM5 12a1 1 0 0 1-1 1H3a1 1 0 1 1 0-2h1a1 1 0 0 1 1 1zm11.95-6.364a1 1 0 0 1 0 1.414l-.707.707a1 1 0 1 1-1.414-1.414l.707-.707a1 1 0 0 1 1.414 0zM8.172 15.778a1 1 0 0 1 0 1.414l-.707.707a1 1 0 1 1-1.414-1.414l.707-.707a1 1 0 0 1 1.414 0zm9.9 1.414a1 1 0 0 1-1.414 0l-.707-.707a1 1 0 0 1 1.414-1.414l.707.707a1 1 0 0 1 0 1.414zM7.05 7.05a1 1 0 0 1-1.414 0l-.707-.707a1 1 0 0 1 1.414-1.414l.707.707a1 1 0 0 1 0 1.414zM12 7a5 5 0 1 0 0 10A5 5 0 0 0 12 7z'/>
                </svg>
              ) : (
            ...
            <button
              ...
              className='w-9 h-9 flex items-center justify-center rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors group'
              aria-label="Search"
            >
              <svg className='w-5 h-5 text-gray-500 dark:text-gray-400 group-hover:text-green-600 transition-colors' fill='none' stroke='currentColor' viewBox='0 0 24 24'>
            ...
            <div className='group relative'>
              <button
                ...
                className='w-9 h-9 flex items-center justify-center rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors group/btn'
                aria-label="Profile"
              >
                {token ? (
                  ...
                  <svg className='w-5 h-5 text-gray-500 dark:text-gray-400 group-hover/btn:text-green-600 transition-colors' fill='none' stroke='currentColor' viewBox='0 0 24 24'>
              ...
              {token && (
                <div className='group-hover:block hidden absolute right-0 top-full pt-2 z-50 min-w-[200px]'>
                  <div className='bg-white/95 dark:bg-gray-800/95 backdrop-blur-xl border border-gray-100 dark:border-gray-700 rounded-2xl shadow-2xl overflow-hidden'>
                    ...
                    <div className='py-1'>
                      {[
                        ...
                        <button key={item.path} onClick={() => navigate(item.path)}
                          className='w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-green-50 dark:hover:bg-green-900/30 hover:text-green-700 dark:hover:text-green-400 transition-colors text-left'>
                          <span>{item.icon}</span>
            ...
            {/* Cart */}
            <Link to='/cart' className='relative w-9 h-9 flex items-center justify-center rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors group' aria-label="Cart">
              <svg className='w-5 h-5 text-gray-500 dark:text-gray-400 group-hover:text-green-600 transition-colors' fill='none' stroke='currentColor' viewBox='0 0 24 24'>
            ...
            <button onClick={() => setVisible(true)}
              className='w-9 h-9 flex items-center justify-center rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors lg:hidden'>
        ...
        {/* Search Bar Dropdown */}
        <div className={`overflow-hidden transition-all duration-300 ${searchOpen ? 'max-h-20 border-t border-gray-100 dark:border-gray-700' : 'max-h-0'}`}>
          <form onSubmit={handleSearch} className='max-w-2xl mx-auto px-4 py-3'>
            <div className='flex items-center gap-2 bg-gray-50 dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-600 focus-within:border-green-400 rounded-xl px-4 py-2 transition-all'>
              ...
              />
              {searchQuery && (
                <button type='button' onClick={() => setSearchQuery('')} className='text-gray-400 hover:text-gray-600 dark:hover:text-gray-200'>✕</button>
              )}
              <button type='submit'
        ...
        <div className={`absolute top-0 right-0 h-full w-72 bg-white dark:bg-gray-900 shadow-2xl transition-transform duration-300 ${visible ? 'translate-x-0' : 'translate-x-full'}`}>
          <div className='flex items-center justify-between p-4 border-b dark:border-gray-700'>
            ...
            <button onClick={() => setVisible(false)}
              className='w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-500 dark:text-gray-400'>✕</button>
          </div>
          <div className='p-4 flex flex-col gap-1'>
            {[
              ...
              <NavLink key={item.to} to={item.to} onClick={() => setVisible(false)}
                className={({ isActive }) => `px-4 py-3 rounded-xl text-sm font-medium transition-colors ${isActive ? 'bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800'}`}>
                {item.label}
              </NavLink>
            ))}
            ...
            <button onClick={toggleTheme}
              className='flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors'>
              {isDark ? '☀️' : '🌙'} {isDark ? 'Light Mode' : 'Dark Mode'}
            </button>
            <div className='border-t border-gray-100 dark:border-gray-700 my-2' />
            <button onClick={() => { setVisible(false); navigate('/buyer') }}
              className='px-4 py-3 rounded-xl text-sm font-semibold text-blue-700 bg-blue-50 dark:bg-blue-900/30 dark:text-blue-300 hover:bg-blue-100 transition-colors text-left'>
              Buyers Portal
            </button>
            <button onClick={() => { setVisible(false); navigate('/seller') }}
              className='px-4 py-3 rounded-xl text-sm font-semibold text-green-700 bg-green-50 dark:bg-green-900/30 dark:text-green-300 hover:bg-green-100 transition-colors text-left'>
              Seller Portal
            </button>
            <button onClick={() => { setVisible(false); navigate('/admin') }}
              className='px-4 py-3 rounded-xl text-sm font-semibold text-gray-700 dark:text-gray-200 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 transition-colors text-left'>
              Admin Panel
            </button>
            {token && (
              <>
                ...
                <button onClick={() => { setVisible(false); logout() }}
                  className='px-4 py-3 rounded-xl text-sm font-semibold text-red-600 bg-red-50 dark:bg-red-900/30 hover:bg-red-100 transition-colors text-left'>
                  🚪 Logout
                </button>
              </>
            )}
